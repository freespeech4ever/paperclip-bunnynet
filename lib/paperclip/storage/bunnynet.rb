  ...
  module Storage
    module Bunnynet
      ...
      def exists?(style_name = default_style)
        ...
        puts "Checking if #{url} exists for style #{style_name}"
      ...
      def public_exists?(style_name = default_style)
        ...
        puts "Checking if #{url} public exists for style #{style_name}"
        http = Net::HTTP.new(url.host, url.port)
        http.use_ssl = true
        request = Net::HTTP::Head.new(url.path)
        response = http.request(request)
      ...
      def copy_to_local_file(style, local_dest_path)
        ...
        puts "Copying to local file #{url} for style #{style}"
        http = Net::HTTP.new(url.host, url.port)
        http.use_ssl = true
      ...
      def flush_writes
        puts "Flushing writes"
        @queued_for_write.each do |style_name, file|
          url = bunnynet_url(style_name)
          puts "Uploading #{url} for style #{style_name}"
      ...
      def bunnynet_url(style_name = default_style)
        puts "Getting Bunny.net URL for #{path(style_name)} for style #{style_name}"
        path = path(style_name)
        URI.parse("#{@bunnynet_storage_url}/#{@bunnynet_storage_zone}/#{path}")
      end

      def public_url(style_name = default_style)
        puts "Getting Bunny.net public URL for #{path(style_name)} for style #{style_name}"
        path = path(style_name)
